...
class Sbox:
	...
	def DivisionTrails(self):
		...
		for i in range(1,self.len):
			...
			input division property = (0,0,1,0,1)
			"""
			output_vectors = []
	...
	def set_POS(self):
		if not self.pos:
			...
			self.pos = qm.simplify_los(ones,dc = [ ], num_bits = 10)
	def get_sbox_constraints(self,X,Y):
		...
		sbox_main = 'predicate sbox(' + ', '.join(temp1) + ' ) =\n'
		fun = [ ]
		for maxterm in POS:
			temp = [ ]
			for i in range(5):
			...
			for i in range(5,10):
				if(maxterm[i] == '1'):
					temp.append ('('+'not '+Y[i-5]+')')
				elif(maxterm[i] == '0'):
					temp.append (Y[i-5])
			fun.append('('+"\\/".join(temp)+')')
		sbox_main =sbox_main + ' /\\ '.join(fun)
		sbox_main = sbox_main + ';' + '\n'
		return (sbox_main)
